...
//			data = text.getBytes("UTF-16LE");
	...
	public boolean write(String text,int type)
	{
		byte[] data = null;
		try
		{
	...
	public boolean writeunicode(String text)
	{
		byte[] data = null;
		try
		{
			data = text.getBytes("UTF-16");
		}
		catch (UnsupportedEncodingException e)
		{
			Log.e("JQ", "Sting getBytes('UTF-16') failed");
			return false;
		}
		if (!write(data, 0, data.length))
			return false;
